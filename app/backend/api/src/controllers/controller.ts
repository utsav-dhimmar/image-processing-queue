...
export const statusCheckController = async (
	...
	if (!statusInfo) {
		return res.status(404).json({
			message:
				"no image processing found with this id or automatically deleted after 30 minutes once proccsing has been comepleted",
		});
	}
	...
	return res.json({
		...
		message:
			"Save Processed image now other wise it will be delete after 30 minutes",
		download_url,
	});
};
